    ...
    <div class="container wp-container">
        ...
        <div class="search_jobpage px-12 py-8 rounded-lg">
            <div class="items-center pb-2">
                ...
                <h1 class="text-dark leading-10 text_34 font-semibold">Nhân viên 
                    <c:choose>
                        <c:when test="${not empty majornameAll}">
                            tất cả
                        </c:when>
                        <c:when test="${empty majornameAll}">
                            ${majorname.name}
                        </c:when>
                    </c:choose>
                </h1>
...
<div id="list_job" class="" >
    <div class="container wp-container">
        <ul class="list-link flex">
            ...
            <li><a href="#" class="text-dark">Việc làm</a> /</li>
            <li><a href="<c:url value="/job-page"/>?major=${majorname.id}" class="text-dark">
                    <c:choose>
                        <c:when test="${not empty majornameAll}">
                            tất cả
                        </c:when>
        ...
        <h1 class="name-content">Việc làm của ngành 
            <c:choose>
                <c:when test="${not empty majornameAll}">
                    tất cả
                </c:when>
                                        ...
                                        <a href="<c:url value="/job-detail/${j[7]}/${j[8]}"/>" class="text-dark flex w-100">
                                                ...
                                                <c:choose>
                                                    <c:when test="${not empty counter}">
                                                        <c:forEach begin="1" end="${Math.ceil(counter/20)}" var="i">
                                                            <li class="page-item"><a class="page-link  text-dark" href="<c:url value="/job-page"/>?page=${i}&major=0">${i}</a></li>
                                                            </c:forEach>
                                                        </c:when>
                                                ...
                                                <div class="group-menu">
                                                    ...
                                                    <ul class="group-menu-list">
                                                        <c:forEach items="${listNameMajor}" var="n">
                                                            <li class="group-menu-items"><a href="<c:url value="/job-page"/>?major=${n.id}">${n.name}</a></li>
                                                            </c:forEach>
                                            ...
                                            <script type="text/javascript">
                                                search_abc = function () {
                                                    ...
                                                    if (inputsearch.value === "" && select.options[select.selectedIndex].value === "0" && selectcity.options[selectcity.selectedIndex].value === "all")
                                                    {
                                                        window.location.href = "/FindingJob/job-page" + "?major=0&city=all&keyword=null";

                                                    }
                                                    if (inputsearch.value === "") {
                                                        window.location.href = "/FindingJob/job-page" + "?major=" + select.options[select.selectedIndex].value
