...
public class JobRepositoryImpl implements JobRepository {
    ...
    public Object[] getDetailJobById(int jobId) {
        ...
        Root rJ = q.from(Job.class);
        Root rC = q.from(Company.class);
        q.where(b.equal(rC.get("id"), rJ.get("companyID")), b.equal(rJ.get("id"), jobId), b.equal(rJ.get("isChecked"), 1),
                 b.equal(rJ.get("isDeleted"), 0));
    ...
    public Job getNameJob(int i) {
        Session s = this.sessionFactory.getObject().getCurrentSession();
        Query q = s.createQuery("From Job where id =: jobID AND isChecked =: checked");
        q.setParameter("jobID", i);
        q.setParameter("checked", 1);
    ...
    public List<Job> getListJobForManageCompany(int company) {
        ...
        q.where(b.equal(rJ.get("companyID"), company), b.equal(rJ.get("isDeleted"), 0));
